...
const Prescriptions = () => {
  ...
  const fetchPrescriptions = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (response && response.data) {
          ...
          const prescriptionData = response.data.map(p => ({
            ...
            doctor: p.doctor || { name: 'Dr. Unknown', specialty: 'General' },
            medications: p.medicine ? [{
              name: p.medicine,
              dosage: '',
              frequency: '',
              duration: ''
          ...
          // Try to fetch doctor information for appointments if not already included
          const hasDoctorInfo = prescriptionData.some(p => p.doctor && p.doctor.name);
          if (!hasDoctorInfo) {
            const appointmentIds = [...new Set(prescriptionData.map(p => p.apId).filter(Boolean))];
            if (appointmentIds.length > 0) {
              console.log('Fetching doctor info for appointment IDs:', appointmentIds);
              // You can add logic here to fetch doctor info if needed
            }
          }
        } else {
          console.log('No prescription data found, using mock data');
          setPrescriptions(MOCK_PRESCRIPTIONS);
          setIsDemoMode(true);
        }
      } catch (apiError) {
        console.error('Error fetching prescriptions:', apiError);
      ...
      toast.error('Failed to load prescriptions. Using sample data instead.');
      setPrescriptions(MOCK_PRESCRIPTIONS);
      setIsDemoMode(true);
    } finally {
      setLoading(false);
