...
const Appointments = () => {
    ...
    try {
      ...
      if (userData && userData !== '{}') {
        try {
          ...
          patientId = user.id || user.P_ID || user.patientId || user.userId;
        } catch (e) {
          console.error('Error parsing user data:', e);
        }
      }
      
      // Fallback to token if no patient ID found
      if (!patientId) {
        try {
          const token = localStorage.getItem('token');
          if (token) {
            const payload = JSON.parse(atob(token.split('.')[1]));
            patientId = payload.id || payload.userId || payload.sub;
      ...
      const formattedAppointments = Array.isArray(response) 
        ? response 
        : (response?.data || []);
  ...
  const handleManualRefresh = useCallback(() => {
    ...
    fetchAppointments().then(() => {
      ...
      const newInterval = setInterval(() => {
        console.log('Auto-refreshing appointments after manual refresh...');
        fetchAppointments(true);
      }, 30000);
      setPollingInterval(newInterval);
    });
  }, [fetchAppointments, pollingInterval]);
  ...
  const isUpcoming = (date, time) => {
    // Check if either date or time is missing or invalid
    ...
    try {
      ...
      if (timeParts.length < 2) return false; // Invalid time format
