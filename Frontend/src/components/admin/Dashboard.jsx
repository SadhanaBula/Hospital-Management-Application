...
const AdminDashboard = () => {
  ...
  // Flatten activities, always use primitive values only
  const activities = [
    ...recentDoctors.map(doc => ({
      type: 'Doctor',
      name: doc.name || doc.drName || 'Unknown',
      date: doc.createdAt || doc.registrationDate || doc.date || '',
      details: doc.specialization?.name || doc.specialty || '',
      email: doc.email || '',
      department: doc.specialization?.name || doc.department || doc.specialty || '',
    ...
    ...recentAppointments.map(app => ({
      ...
      department: (app.doctor && app.doctor.specialization?.name) || app.department || '',
    })),
            ...
            {recentDoctors.length === 0 ? (
              ...
              recentDoctors.map(doc => (
                <li key={doc.id || doc.drId} className="py-2 flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-500">{doc.specialization?.name || doc.specialty || ''}</span>
                </li>
              ))
            )}
          </ul>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <ul className="divide-y divide-gray-200">
            {recentPatients.length === 0 ? (
              ...
              recentPatients.map(pat => (
                <li key={pat.id || pat.pId} className="py-2 flex items-center justify-between">
