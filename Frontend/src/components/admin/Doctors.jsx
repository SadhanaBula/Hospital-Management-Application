...
const Doctors = () => {
  ...
  const specialtyMap = specialties.reduce((map, specialty) => {
    map[specialty.spId ?? specialty.id] = specialty.spName ?? specialty.name;
    return map;
  }, {});
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await doctorService.deleteDoctor(doctorToDelete.drId ?? doctorToDelete.id);
  ...
  const filteredDoctors = doctors.filter((doctor) => {
    ...
    let doctorSpecialtyId =
      doctor.specialization?.spId ?? doctor.specialization?.id ?? doctor.spId;
    const matchesFilter =
      selectedFilter === 'all' || String(doctorSpecialtyId) === selectedFilter;
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {isLoading ? (
              ...
              filteredDoctors.map((doctor) => {
                ...
                const email = doctor.email ?? '';
                const mobileNo = doctor.mobileNo ?? '';
                const experience = doctor.experience ?? '';
                const specialtyName =
                  specialtyMap[
                    doctor.spId ??
                      (doctor.specialization && (doctor.specialization.spId ?? doctor.specialization.id))
                  ] || doctor.specialization?.name || 'Unknown';

                return (
                  <tr key={doctor.drId ?? doctor.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">{mobileNo}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {experience} years
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button className="text-red-600 hover:text-red-900 ml-4" onClick={() => handleDelete(doctor)}>
